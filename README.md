...
本节描述第一轮 Phase 0-1 范围：Java 17 + Maven 脚手架、CLI 参数解析、项目 YAML 配置加载、基础日志、JUnit 5 测试，以及本地 sealed 文件扫描器。
...
后续 Phase 会继续实现实际本地 GC 删除、限流/退避、指标、HadoopHdfsClient、MiniDFSCluster 和本地伪分布式 HDFS E2E。当前版本仍不实现 HadoopHdfsClient，也不会在 manifest 成功后删除本地 sealed 文件。

## Phase 5-6 当前实现

当前版本继续补齐 LocalFs 模式的 manifest 与 worker 串联：

- `ManifestWriter` 定义本地 manifest 写入接口；
- `JsonlManifestWriter` 写入本地 JSONL manifest，并按 `file_id` 提供 `writeIfAbsent` 语义；
- 重复写入完全一致的 manifest 记录会幂等成功；同一 `file_id` 内容不一致会报错；
- `UploaderWorker` 串联 scanner、metadata、WAL state、LocalFs commit protocol 和 manifest writer；
- HDFS final 成功但 manifest 写失败时，状态会保留/恢复为 `COMMITTED_TO_HDFS`，不会进入本地 GC；
- manifest 写成功后状态进入 `MANIFEST_COMMITTED`，随后进入 `LOCAL_GC_READY`，但当前版本仍不会删除本地文件。
...
1. `/tmp/fake_hdfs` 下只有一个 final 文件；
2. 第二次运行日志中出现 final 已存在且 checksum 一致的幂等成功信息；
3. manifest 不产生重复冲突；
4. `/tmp/trace_spool/state/upload-state.jsonl` replay 后最终状态可恢复到 `LOCAL_GC_READY`。
